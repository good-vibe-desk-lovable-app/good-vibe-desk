...
import { ChevronDown, ExternalLink, Lightbulb, Search, Tags } from "lucide-react";
import { useMemo, useState } from "react";

import { Collapsible, CollapsibleContent, CollapsibleTrigger } from "@/components/ui/collapsible";
import { BackLink, Eyebrow, Fact, PackFeedback } from "@/components/ui/compendium-helpers";
  ...
  if (loading || error) {
    return (
      <PackFeedback
        ...
        error={error}
        icon={Lightbulb}
        packName="Technologies Compendium"
        loadingBody="Preparing the cached offline technology directory."
      />
    );
  }
      ...
      <section className="rounded-2xl border border-sky-400/25 bg-card/80 p-5 shadow-sm backdrop-blur sm:p-7">
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div className="max-w-2xl">
            <Eyebrow icon={<Lightbulb className="size-3.5" />}>
      ...
      >
        ...
        {records.length > 0 ? (
          ...
          <EmptyState
            ...
            body="Try searching for a different technology or selecting another category."
          />
        )}
      </section>
    </PageShell>
