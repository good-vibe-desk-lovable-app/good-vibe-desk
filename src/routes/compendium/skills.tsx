  ...
  if (loading || error || !knowledge || !filteredData) {
    return (
      <PackFeedback
        ...
        error={error}
        icon={Sparkles}
        packName="Skills Compendium"
        loadingBody="Preparing the cached offline skills & passives directory."
      />
    );
  }
      ...
      <Collapsible className="mt-5 rounded-xl border bg-card/60 p-4">
        ...
        <CollapsibleContent className="mt-3 text-xs leading-relaxed text-muted-foreground space-y-2">
          <p>
            ...
            learnset rows, 299 partner skills, 53 guaranteed passives, 320 active inheritance rules,
            and 85 passive inheritance rules.
          </p>
          <p>
            <strong>Catalogue vs Roster Distinction:</strong> The 395 active catalogue rows include
            NPC, boss, and unused code entries, while the Pal roster uses 307 distinct natural
            learnset names. Similarly, the 412 passive catalogue rows contain internal and special
            entries alongside 115 player-obtainable passives.
          </p>
          <p>
            <strong>Availability vs Inheritability:</strong> Natural learnsets and natural rolling
            pools determine what a Pal naturally possesses, whereas breeding rules and Skill Fruits
            govern what can be inherited or taught.
          </p>
          <p>
      ...
      >
        ...
        {totalFilteredCount === 0 ? (
          <EmptyState
            ...
            body="No skills, passives, learnsets, or rules match your search or filters. Try adjusting your search query."
          />
        ) : (
            ...
            {showLearnsets && filteredData.learnsets.length > 0 && (
              <div>
                ...
                <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
                  Shows which active move each Pal species learns naturally as it levels up in-game.
                </p>

                <div className="mt-4 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                  {filteredData.learnsets.slice(0, 150).map((row, idx) => (
                    <PalLearnsetCard key={`${row.palInternalName}-${row.name}-${idx}`} row={row} />
                  ))}
                </div>
                    ...
                    <div>
                      ...
                      <div className="grid gap-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                        {filteredData.activeInheritance.slice(0, 100).map((rule, idx) => (
                          <ActiveInheritanceCard key={`${rule.internalName}-${idx}`} rule={rule} />
                        ))}
                      </div>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div className="grid gap-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                        {filteredData.passiveInheritance.map((rule, idx) => (
                          <PassiveInheritanceCard key={`${rule.internalName}-${idx}`} rule={rule} />
                        ))}
                      </div>
                    </div>
                  )}
